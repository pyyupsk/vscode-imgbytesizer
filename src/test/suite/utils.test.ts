...
suite("Utils Test Suite", () => {
  ...
  suite("getImgbytesizerPath()", () => {
    ...
    test("Should trim whitespace from configured path", () => {
      ...
      mockWorkspaceConfig.get
        .withArgs("imgbytesizerPath")
        .returns(customPathWithSpace);
  ...
  suite("checkImgbytesizerInstalled()", () => {
    ...
    test("Should return true if imgbytesizer check command succeeds", async () => {
      ...
      assert.ok(
        execSyncStub.calledOnceWith("imgbytesizer -v", { stdio: "ignore" }),
      );
    });
  ...
  suite("buildCommand()", () => {
    ...
    setup(() => {
      ...
      getImgbytesizerPathStub = sandbox
        .stub(utils, "getImgbytesizerPath")
        .returns("imgbytesizer");
    });
  ...
  suite("runImgbytesizer()", () => {
    ...
    setup(() => {
      ...
      execSyncStub = sandbox.stub(require("child_process"), "execSync");
      buildCommandStub = sandbox
        .stub(utils, "buildCommand")
        .returns("mocked command to execute");
    ...
    test("Should create output directory if it does not exist", async () => {
      ...
      existsSyncStub
        .withArgs(path.dirname(defaultMockedOutputPath))
        .returns(false); // Output dir doesn't exist
    ...
    test("Should use explicitly provided outputPath in options", async () => {
      ...
      const result = await utils.runImgbytesizer(
        testImagePath,
        optionsWithOutput,
      );
