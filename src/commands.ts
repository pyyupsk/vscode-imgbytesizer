 ...
 */
async function showFormatQuickPick(
  originalExt: string,
...
export async function resizeImage(uri?: vscode.Uri): Promise<void> {
  try {
    ...
    const options: utils.ImgByteOptions = {
      ...
      minDimension:
        defaultOptions.minDimension === 0
          ? undefined
          : defaultOptions.minDimension,
...
export async function resizeImageWithOptions(uri?: vscode.Uri): Promise<void> {
  try {
    ...
    const exactSizeResult = await vscode.window.showQuickPick(
      exactSizeOptions,
      {
